...
To keep a source directory in sync, open an indexed KB's **Linked folders** section, enter a path accessible to the DeepTutor server, and choose **Link folder**, then **Sync now**. In Docker, use the directory's mounted path inside the container. The section shows indexing progress and the last successful sync. **Upload folder** remains a one-time import; linking a source directory enables later manual imports of new and modified files. Unlinking or deleting a source file does not remove already imported documents from the KB.
